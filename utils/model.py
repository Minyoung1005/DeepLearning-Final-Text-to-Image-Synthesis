...
class G_NET(nn.Module):
    def __init__(self):
        ...
        if cfg.TREE.BRANCH_NUM > 0:
            self.h_net1 = INIT_STAGE_G(ngf * 16, ncf).to(self.device)
            self.img_net1 = GET_IMAGE_G(ngf).to(self.device)
        # gf x 64 x 64
        if cfg.TREE.BRANCH_NUM > 1:
            self.h_net2 = NEXT_STAGE_G(ngf, nef, ncf, 64).to(self.device)
            self.img_net2 = GET_IMAGE_G(ngf).to(self.device)
        if cfg.TREE.BRANCH_NUM > 2:
            self.h_net3 = NEXT_STAGE_G(ngf, nef, ncf, 128).to(self.device)
            self.img_net3 = GET_IMAGE_G(ngf).to(self.device)
...
class D_GET_LOGITS(nn.Module):
    ...
    def forward(self, h_code, c_code=None):
        if self.bcondition and c_code is not None:
            ...
            # state size (ngf+egf) x 8 x 8 # 4 x 4
